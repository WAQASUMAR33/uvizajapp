...
export async function POST(req: NextRequest) {
  ...
  const { customerId, merchantId, rating, review } = body ?? {};
  ...
  const merchantIdInt = parseInt(merchantId);
  const customerIdInt = parseInt(customerId);

  const merchant = await prisma.merchant.findUnique({ where: { id: merchantIdInt }, select: { id: true } });
  if (!merchant) {
  ...
  const ratingRecord = await prisma.merchantRating.upsert({
    where:  { customerId_merchantId: { customerId: customerIdInt, merchantId: merchantIdInt } },
    update: { rating, review: review ?? null },
    create: { customerId: customerIdInt, merchantId: merchantIdInt, rating, review: review ?? null },
  });
